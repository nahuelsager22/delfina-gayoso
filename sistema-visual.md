...
### 7.2 bis — El nombre de la propuesta (Bloque 8 · 34ª ola)

**Problema, en dos tiempos.** Primero el nombre del servicio estaba en `--text-meta` y tinta suave, **dos escalones por debajo de su propio subtítulo**: el ojo entraba por la descripción. Se subió a `--text-titulo` en caja alta, tinta plena y con un filete, y ganó jerarquía — pero no identidad: seguía siendo Karla en mayúsculas, o sea **el mismo gesto que el rótulo de la sección** un cuerpo más grande. Se leía como un kicker de segundo nivel.

**Lo que el sistema reserva para cada cosa** (la evidencia que decidió la solución):

| Tratamiento | Qué nombra hoy |
|---|---|
| `voz-display` (Fraunces, WONK on) | Lo que **tiene nombre propio**: títulos de sección, marcas de la cinta, colaboraciones, clases pasadas, el monograma |
| Karla medium `text-titulo` | **Ítems de catálogo**: ebooks, capítulos, próximas experiencias |
| El acento como tinta | **Rótulos y metadatos**: `.proxima-rotulo`, `.bloque-clases-rotulo`, `.pagina-rotulo`, modalidad, sello de estado |

Un servicio no es un SKU —son las tres cosas que hace Delfina—, así que pertenece a la primera fila. Por lo mismo se **descartó diferenciarlo por color**: pintar el nombre con el acento lo devuelve a la categoría de la que se lo quiere sacar, un escalón más arriba. Y se descartó una **tercera familia**: §2.1 fija dos familias con dos funciones semánticas, y los ejes de Fraunces dan lo que hacía falta.

**Solución: el nombre es su voz, dicha con firmeza.** Fraunces `voz-display`, caja baja, y el **peso** haciendo la jerarquía en lugar del tamaño.

- **`--font-weight-nombre`, un token nuevo.** El eje `wght` de Fraunces está entero y disponible desde el Bloque 5 (`fonts.ts` lo conserva a propósito) y **nunca se había usado**: todo el serif del sitio es regular. Por eso es a la vez del sistema y distinto de todo lo que hay.
- **El cuerpo BAJA.** El nombre queda en `--text-titulo`, por debajo de la descripción que le sigue, y aun así manda: lo que el ojo compara es el trazo.
- **Caja baja.** Todos los rótulos del sitio son mayúsculas; salir de la caja alta corta de raíz el parecido con el kicker. Efecto colateral: en 390px los tres nombres entran en UNA línea, cosa que en mayúsculas no pasaba.
- **WONK on**, las terminaciones irregulares que Fraunces sólo permite en cuerpos de display. Es lo más de ella que da la tipografía.
- **El filete y la tinta plena se conservan** de la primera mitad de la ola: el filete es la señal de escaneo y el acento ahí es color de marca, no de tinta.

**LA REGLA QUE DEJA — la escalera de TRAZO.** En una sala, el orden de lectura se mide por grosor de trazo, no por cuerpo, y **ningún nombre puede pesar más que la habitación que lo contiene**. Medido (ancho del palo de la «l», normalizado al cuerpo real):

| | 390px | 768px | 1440px |
|---|---|---|---|
| título de sección | 2,44 | 3,29 | 4,80 |
| **nombre del servicio** | **2,29** | **2,59** | **3,90** |
| descripción | 1,77 | 2,12 | 2,72 |
| rótulo de sección | 1,12 | 1,15 | 1,20 |

Esa escalera es lo que obligó a que **el peso sea responsive** (600 abajo, 700 desde 900px): el título de sección encoge más rápido que el nombre —su clamp arranca en 30px y el del nombre en 19—, así que con 700 fijo el nombre pasaba a tener más trazo que la sala en mobile (2,85 contra 2,44). También es lo que descartó subir el nombre a `--text-voz-l`: ahí su trazo habría llegado a 4,84 contra los 4,44 de la sección, invirtiendo la jerarquía de la habitación.

**Verificado** en 390, 768, 1280 y 1440: escalera correcta en los cuatro, los tres nombres en una línea, sin overflow. El eje `wght` se comprobó sirviéndose de verdad (Fraunces llega con rango `100 900` y el ancho del texto varía con el peso).
...
- **Arquitectura de atmósferas — Opción C: ancladas con transición (decisión).** Se evaluaron dos aproximaciones: (A) campo continuo que interpola siempre entre centros de sección —la actual—, y (B) atmósferas ancladas a cada sección con transición en los bordes (filosofía Brenda). Diagnóstico: la sensación de "todo vive en el mismo crema" venía en gran parte de A —al promediar constantemente, ninguna atmósfera llega a su fuerza plena—. Se eligió una **Opción C** (superior y no más compleja): cada atmósfera se **sostiene a fuerza plena mientras se recorre su sección** (meseta `HOLD = 0.36` en cada extremo del tramo entre centros) y sólo **cruza suave en la banda de borde**. Da la identidad distinta de B + la continuidad de A. El motor lo resuelve remapeando el factor de interpolación (no es un rediseño). Motivo de la elección: es la que mejor produce "cada capítulo, un recuerdo emocional distinto" sin cortes, con la implementación más simple.
- **Atmósferas con más presencia.** Ahora que cada una llega completa, se subió la intensidad y personalidad: manteca 0.26, oro 0.36, **corazón corteza 0.42 + café 0.24**, piedra 0.30, **verde perejil 0.34**, mesa dorada 0.32, atardecer corteza 0.34. Temperaturas claramente distintas (verificado en el centro de cada sección); el texto sigue en `Hierro`, legible.
- **Menú mobile — hoja atmosférica opaca (sin transparencia).** Con el menú abierto, el bar deja de desvanecerse y se vuelve **opaco**, y el panel pasa a ser una **superficie de pantalla completa** (`position: fixed; inset: 0; 100dvh`) **opaca**, coloreada por la MISMA atmósfera del punto del recorrido (base `--atm-navbar-rgb` + focos de `--atm-color`/`--atm-prof`). Cubre el contenido (no se ve nada detrás); bar + hoja son una sola pieza del mismo universo. Nada de transparencias.

### Continuación (octava ola de indicaciones, 2026-07-11)

- **Jerarquía — portadas de capítulo.** Los grandes momentos anuncian su entrada con un encabezado visible y distintivo: rótulo corto (sans, meta, uppercase, `--space` de aire) + título en **serif display grande** (`.momento-titulo`, `--text-voz-xl`). Se aplica a "Lo que te podés llevar" (kicker "Ebooks y clases") y "Trabajemos juntos" (kicker "Servicios"); la serie mantiene su "La serie / Cocina Nivel 0". Además cada ficha lleva un **rótulo de categoría** ("Ebook" / "Clase en vivo") y cada servicio su `tipo` — se distinguen de un vistazo. El `h2` visible reemplaza al sr-only cuando hay portada (estructura intacta).
- **Atmósferas — identidades más reconocibles (rompe Yema/Corteza).** Paleta ampliada (chocolate, café, madera, terracota/especias, vino, oliva/musgo, piedra/hierro, manteca). Cada momento, una temperatura distinta: manteca → **oro** (ebooks) → **chocolate** (aprendizaje) → **piedra fría** (quién soy) → **verde** (servicios) → **terracota/especias** (comunidad) → **vino/atardecer** (cierre). Comportamiento variado: algunas abrazan casi todo el ancho (radio 88–96, ebooks/aprendizaje/verde/comunidad), otras más sutiles (piedra). Legibilidad verificada (texto `Hierro`).
...
Indicaciones directas de Delfina, con prioridad sobre decisiones previas (ver la sección homónima en `direccion-de-arte.md` para la justificación completa y la reconciliación con la tesis de color).

> **Tercera ola (misma fecha) — SALAS EDITORIALES (estado final del código).** Se abandona el campo continuo interpolado (`.campo-atmosferico` + motor) por un modelo editorial: cada `<section>` es una habitación full-bleed con su color propio, estático (no "enciende" al llegar). Detalle técnico:
> - `config.ts` reescrito a `SALAS` (`bg` gradiente, `ink`/`inkSoft`/`accent`/`navBg`/`oscura`) + `estiloSala()` (fija `background`, `color` y `--atm-ink`/`--atm-ink-soft`/`--atm-accent` scopeadas en el `<section>`, que el contenido hereda) + `escribirNavbar()`.
> - `Momento.tsx` reescrito: `<section>` full-bleed con `estiloSala` + `overflow-x: clip` + **borde de onda** SVG (curva orgánica entre salas); el contenido se centra en `.sala-inner` (max 80rem + padding). `page.tsx`: `<main>` a ancho completo, salas contiguas (se quitaron los adornos de gap).
> - `AtmosferaProvider` ahora sólo actualiza la tinta del **navbar** (`--nav-ink`/`--nav-ink-soft`/`--nav-accent`/`--nav-bg`) según la sala que tiene debajo; sin capa fija. El navbar y el menú mobile consumen `--nav-*`.
> - **Contraste por sala** (tinta sobre su fondo sólido): principal 4.3–12 (≥AA, varias AAA), secundario ≥AA, acento decorativo. Islas claras (`.cap-paper`) reasignan tinta oscura.
> - Componente nuevo `Sello` (sello con **texto en trayectoria**, SVG `textPath`). El CTA `Yema` gana borde en la tinta de la sala (se separa sobre la habitación oro). Rescoldo de MasterChef estático (local en la sala íntima).
> - **Responsive verificado** (360/768/desktop): sin overflow-x, navbar/menú/salas por formato.
>
> _(La nota de la 2ª ola de abajo —campo continuo + tinta interpolada— quedó superada.)_
>
> **Octava ola — paleta editorial + refinamientos.** (1) **PALETA reencuadrada sobre el manual de marca**: se retira el oro/naranja saturado (leía lúdico) y entran ARENA `#DCCBA6` (ebooks), MARRÓN `#4A3527` (aprendizaje), TAUPE `#AEA391` (comunidad), VERDE BOSQUE `#39532A` (servicios), VINO apagado `#4E2630` (cierre), sobre crema; hero y "quién soy" sin bloque. Acento global → ocre `#6B4A1F`; el CTA de compra pasa del oro al **verde de marca** con tinta crema; el `#` del numeral usa el acento de la sala. Contraste medido: tinta 6.2–11.2 (≥AA, mayoría AAA). (2) Marquesinas con **guarda** (cenefa de arcos arriba/abajo) y tonos nuevos (marrón/verde/vino/taupe). (3) Vuelven los **adornos** en los respiros de crema entre bloques. (4) **La Serie**: `.serie-capitulos` con más aire, filete entre capítulos y zigzag sólo ≥900px — en mobile todos alinean a la izquierda (se corrige el desplazamiento). (5) **Cierre**: el bloque se estira para ocupar el alto disponible (de debajo del navbar al borde inferior) con el contenido centrado — sin vacío ni contenido flotando. (6) **Clase en vivo** separada de los ebooks en su propia superficie (`.bloque-clases`) con rótulo propio. Verificado desktop 1440×900 y mobile 375: sin overflow-x, capítulos alineados, cierre lleno.
>
> **Séptima ola — composición editorial (fondo crema + bloques de color).** Cambio de fondo del sistema visual, siguiendo las referencias: el fondo del sitio es CREMA (`Harina`); los momentos importantes (`sala.panel` en `config.ts`: ebooks, aprendizaje, servicios, comunidad, cierre) se muestran en un BLOQUE de color contenido (`.sala-panel`: rectángulo con radio y marco fino, márgenes crema); hero y "quién soy" viven sobre crema. `Momento` reescrito (sin ondas, sin fondo full-bleed). Consecuencias: (1) Marquesinas = banda RECTA sobre crema (Opción A), con filete fino arriba/abajo; texto recto. (2) Ya no hay uniones color-a-color → desaparece el "corte/difuminado" de las ondas (no aplican). (3) Navbar = tinta fija Hierro sobre `Harina` (vive siempre sobre crema); `AtmosferaProvider` queda como envoltura neutra. (4) Ilustraciones de comunidad → `--atm-ink-soft` (crema legible sobre el panel terracota, antes desaparecían en `Corteza`). (5) Cierre = bloque vino contenido y COMPACTO (tipografía/interlineado/gaps reducidos) que entra ~en el viewport, con el logotipo. (6) Loading: la respiración pasó a una entrada FINITA (no quedaba animando en segundo plano). Verificado desktop/mobile: sin overflow-x, panels contenidos (311px en 375), cierre compacto, ilustraciones legibles.
>
> **Sexta ola — refinamiento + identidad de marca.** (1) Hero: se quita el subrayado de "conmigo" (la línea ilustrada ya da ese énfasis). (2) Marquesinas (Opción B): texto RECTO legible (marquee CSS) + fondo ONDULADO (onda superior propia con degradado + onda inferior de la sala siguiente) → integradas al sistema de ondas. (3) Ondas del fondo: el relleno de cada onda es un DEGRADADO VERTICAL con el tope translúcido (`stop-opacity` 0→1), que se funde con el color de arriba creando un tono intermedio (transición continua, sin "corte"; prev-agnóstico) — reemplaza el relleno sólido. (4) Cierre: se reemplaza el sello por el LOGOTIPO OFICIAL (`/logotipo/logotipo-1.png`, disco crema sobre el vino), protagónico, con `.cierre-grid` anclado al fondo (`align-items:end`) → sin espacio vacío. (5) Open Graph: `metadataBase` + `openGraph.images` = `/logotipo/logotipo-3.png`. (6) Loading: `.pantalla-carga` (en el HTML inicial, sin flash) = logotipo sobre `Harina` con respiración suave, auto-dismiss por CSS; con `prefers-reduced-motion` se retira de inmediato. Verificado desktop/mobile: sin overflow-x, cierre y marquesina apilan/centran bien.
>
> **Quinta ola — composición editorial.** (1) `Momento` gana modo `full` (sala a 100svh con el contenido centrado): **hero** y **cierre** son spreads horizontales que no caen en vertical. (2) **Hero** (Umbral): `.hero-grid` de 2 columnas ≥900px (texto | foto), `EspacioFoto` generoso en arco (ratio 4/5, respira, será protagonista), sello superpuesto; `voz-hero` reajustado a `clamp(2.05rem, 4.4vw, 3.4rem)` para no desbordar. (3) **Cierre** rediseñado como espejo del hero (texto | sello grande), sin hueco vacío. (4) **Navbar** interpola su color entre salas en una banda de 140px alrededor del borde (`AtmosferaProvider`), en vez de saltar: no "cambia de tema". (5) **Marquesinas** integradas al sistema de ondas (borde superior curvo con la misma geometría, `--mq-fill`). (6) **Vino** más profundo (`#5f1e2c`, antes se lavaba con Harina; AAA) y borde de onda sin corte (curva continua de extremo a extremo). Responsive verificado (mobile/desktop): sin overflow-x, hero apila la foto, cierre centrado. _(Nota: el cache `.next` puede servir CSS viejo tras editar `globals.css`; si un cambio no se refleja, reiniciar el dev server.)_
>
> **Cuarta ola — recursos editoriales.** Sobre el modelo de salas: (1) **Hero** rediseñado (Umbral): titular `.voz.voz-hero` (clamp 2.7→5.4rem, sin `@layer` para ganar a la utilidad), línea a mano, y `EspacioFoto` (hueco reservado para la foto real, en arco, se le pasa la imagen como `children` cuando llegue) + `Sello` superpuesto, composición asimétrica. (2) **Botón** `.cta-producto` es ahora un recurso gráfico: radio orgánico asimétrico + **sombra dura** (sticker) en la tinta de la sala + **flecha dibujada** (`Flecha`) que se adelanta al hover + microinteracción de "apriete". (3) **Marquesina** (`Marquesina`): banda de color a pleno ancho con texto serif en movimiento (color como composición + ritmo; se congela con reduced-motion). (4) Primitivos nuevos: `Flecha`, `LineaEditorial`, `EspacioFoto`, `.enlace-flecha`. (5) **Cierre** (LaClaseNoTermina) rediseñado editorial: titular hero + línea + sello de despedida + enlaces con flecha, asimétrico. (6) **Color más saturado**: verde `#2f5a2c` y vino `#6a2233` (antes se lavaban con Harina), contraste re-medido (ink 7.0 / 9.3, AAA). Responsive verificado (mobile/tablet/desktop): overflow-x 0, hero apila la foto, marquesina recortada.

- **Sistema de Atmósferas v2 (reescribe `config.ts`).** El `Atmosfera` gana una capa `base` (relleno plano que cubre todo el campo) y su **tinta** (`ink`, `inkSoft`, `accent`). El campo (`.campo-atmosferico`) suma una capa de base plana bajo los focos. Galaxias **luminosas** (base baja, tinta oscura): bienvenida/manteca (base 0.16, color 0.42), calida/oro (0.18, 0.58), íntima/piedra (0.12, 0.34), compartir/terracota (0.20, 0.46). Galaxias **profundas** (base casi opaca, tinta clara): corazón/chocolate (base `#4A2D25` 0.88), fresca/**verde de marca** (`#2B4222` 0.90), despedida/vino (`#682830` 0.85). Cada una con su `luz`/`color`/`profundidad`.
- **Tinta adaptativa — variables nuevas `--atm-ink` / `--atm-ink-soft` / `--atm-accent`** (tripletes RGB que el motor interpola y escribe en `:root`). El texto base (`html`), la voz, los títulos, kickers, metas, el navbar y su menú mobile, el medidor de wayfinding, los enlaces y los acentos dibujados las consumen (`rgb(var(--atm-ink, …))`, con fallback = galaxia luminosa). Islas claras (`.cap-paper`, la hoja de recetario) reasignan `--atm-ink` a oscuro localmente. `INK_OSCURA_SOFT` se oscureció a `#3E362D` para leer sobre los campos cálidos saturados.
- **Contraste medido (compositing sobre `Harina`, punto más claro = base+luz y más hondo = base+color+prof).** Texto **primario ≥AA en las 7 galaxias** (worst 4.76 vino, hasta 12 manteca; AAA realista); **secundario ≥4.2** en el peor punto del foco. `Bosque/Harina` = 7.71 (AAA). Aura MasterChef sobre `íntima` = 4.73 (AA). La prioridad de lectura se sostiene sin limitar el color: la tinta se adapta.
- **Enriquecimiento — componentes nuevos (§7).** `Conector` (`app/_chrome/adornos/Conector.tsx`): curva vertical que enhebra un bloque con el siguiente, marca la entrada a las galaxias, trazo en acento adaptativo, dibujada al entrar en viewport. `MarcoFoto` (`.../MarcoFoto.tsx`): dos esquinas curvas dibujadas que abrazan las portadas de ebooks (curvas alrededor de la foto, sin card). `Adorno`, `CirculoAnotado`, `SubrayadoAnotado` pasan su default al **acento adaptativo** (corteza/oro según el paisaje). Utensilios del navbar y medidor: tinta adaptativa.
